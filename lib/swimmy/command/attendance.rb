  ...
  module Command
    class Attendance < Swimmy::Command::Base
      command 'hi', 'bye' do |client, data, match|
        ...
        when 1
          # if the person specified in the argument is active
          user_name = arg
        when -1
          # if the person specified in the argument is not active
      ...
      def self.parse_arg(arg)
        # judge if the person is specified in the argument
        active_members = spreadsheet.sheet("members", Swimmy::Resource::Member).fetch.select {|m| m.active? }.map {|m| m.account }

        case arg
        in nil
          # return 0 if arg is not specified
          return 0
        in String => s if active_members.include?(s)
          # return 1 if the person specified in the argument is active
          return 1
        else
          # return -1 if the person specified in the argument is not active
          return -1
        end
